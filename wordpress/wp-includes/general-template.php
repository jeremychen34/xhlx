...
function wp_get_document_title() {
	...
	// If it's a date archive, use the date as the title.
		...
		$title['title'] = get_the_date( _x( 'm Y', 'monthly archives date format' ) );
...
function get_the_archive_title() {
	if ( is_category() ) {
		...
		$title = sprintf( __( 'Month: %s' ), get_the_date( _x( 'm Y', 'monthly archives date format' ) ) );
	} elseif ( is_day() ) {
		/* translators: Daily archive title. 1: Date */
		$title = sprintf( __( 'Day: %s' ), get_the_date( _x( 'm j, Y', 'daily archives date format' ) ) );
	...
	if ( 'monthly' == $r['type'] ) {
		...
		if ( $results ) {
			...
			foreach ( (array) $results as $result ) {
				...
				$text = sprintf( __( '%1$s %2$d' ), zeroise($result->month,2).'月', $result->year );
...
function get_calendar( $initial = true, $echo = true ) {
	...
	for ( $day = 1; $day <= $daysinmonth; ++$day ) {
		...
		if ( in_array( $day, $daywithpost ) ) {
			// any posts today?
			$date_format = date( _x( 'm j, Y', 'daily archives date format' ), strtotime( "{$thisyear}-{$thismonth}-{$day}" ) );
